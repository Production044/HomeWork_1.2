...
"""
Був зроблений Record як абстрактний класс"""
...
class Record(ABC):  # Making Record an abstract class
    @abstractmethod
...
class ConcreteRecord(Record):  # Implementing concrete methods
...
class AddressBook(UserDict):
    ...
    def find_next_weekday(d, weekday):
        """
        Function to find the next specified weekday after the given date.
        d: datetime.date - initial date.
        weekday: int - weekday from 0 (Monday) to 6 (Sunday).
        """
        days_ahead = weekday - d.weekday()
        if days_ahead <= 0:  # If birthday has already passed in this week.
            days_ahead += 7
...
def add_contact(args, book: AddressBook):
    ...
    if record is None:
        record = ConcreteRecord(name)  # Use ConcreteRecord instead of Record
